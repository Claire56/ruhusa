...
This document records the Ruhusa v0.6.0 trust model, attacker capabilities, security invariants, tested threats, implemented controls, confirmed gaps, and known limitations at the release boundary.

The prior v0.5 snapshot remains frozen at:

```text
docs/threat-model/v0.5.md
```

This file is the frozen v0.6.0 snapshot:

```text
docs/threat-model/v0.6.md
```

Do not rewrite either historical snapshot after release. Later security changes
must be documented in the living threat model and a new versioned snapshot.
